...
After loading the Kinshasa shapefile, I create buffers first using the standardized measure of the communes' areas (`r names(kinshasa_sf[14])`) and then taking a portion of the areas by multiplying each by $10^{-6}$ and adding 2km buffer (`r names(kinshasa_sf[15])`). Throughout the analysis, I am using (`r names(kinshasa_sf[15])`), which has the maximum value `r as.integer(max(kinshasa_sf$buffer2))` for the commune `r kinshasa_sf$name[which.max(kinshasa_sf$buffer2)]` , and minimum value `r as.integer(min(kinshasa_sf$buffer2))` for the commune `r kinshasa_sf$name[which.min(kinshasa_sf$buffer2)]`.
