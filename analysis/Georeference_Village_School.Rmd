...
```{r}
df1 <- read_excel(paste((file.path(here::here(current_path) %>% dirname() %>% dirname(), "output")),'matching_buffers4_20km.xlsx',sep="/"), col_types="text")
df2 <- read_excel(paste((file.path(here::here(current_path) %>% dirname() %>% dirname(), "output")),'matching_buffers4_25km.xlsx',sep="/"), col_types="text")
df3 <- read_excel(paste((file.path(here::here(current_path) %>% dirname() %>% dirname(), "output")),'matching_buffers4_30km.xlsx',sep="/"), col_types="text")
df4 <- read_excel(paste((file.path(here::here(current_path) %>% dirname() %>% dirname(), "output")),'matching_buffers4_35km.xlsx',sep="/"), col_types="text")
df5 <- read_excel(paste((file.path(here::here(current_path) %>% dirname() %>% dirname(), "output")),'matching_buffers4_40km.xlsx',sep="/"), col_types="text")
unique1 <- unique(df1$ceni_site_vote_ID)
unique2 <- unique(df2$ceni_site_vote_ID)
unique3 <- unique(df3$ceni_site_vote_ID)
unique4 <- unique(df4$ceni_site_vote_ID)
unique5 <- unique(df5$ceni_site_vote_ID)
...
rm(list=b)
`%notin%` <- Negate(`%in%`)
df <- read_excel(paste((file.path(here::here(current_path) %>% dirname() %>% dirname(), "output")),'matching_buffers3_30km.xlsx',sep="/"), col_types="text")
df[,8:11] <- lapply(df[,8:11], as.numeric)
df$ceni_site_vote_ID <- as.numeric(df$ceni_site_vote_ID)
...
matching_binded1 <- do.call(bind_rows, lapply(b, get, env=environment()))
matching_binded1 <- matching_binded1 %>% dplyr::mutate(source = "adm4_20km")
matching_adm1$ceni_site_vote_ID <- as.numeric(matching_adm1$ceni_site_vote_ID)
matching_final1 <- dplyr::bind_rows(matching_adm1, matching_binded1)
