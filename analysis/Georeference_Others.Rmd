...
sigeschools.all <- read_excel(paste((file.path(here::here() %>% dirname(), "raw")),'sigeschools.xlsx', sep="/"))
...
for(s in small_villes) {
  ...
  sigeschools.all_sf_admin <- st_join(sigeschools.all_sf,  temp_villes_sf_buffer_10km, left = FALSE) %>%
    ...
    dplyr::distinct(NomEcole, town_province.y, .keep_all = TRUE) # HERE I REMOVE DUPLICATE WITHIN SAME VILLE
